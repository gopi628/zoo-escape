...
func _ready():
	...
	spawn_player()
	# Connect UI buttons
	$UI/HBoxContainer/RestartButton.connect("pressed", Callable(self, "_on_restart"))
	next_button.connect("pressed", Callable(self, "_on_next"))
	# Load first level
...
func spawn_player():
	...
	add_child(player)
	player.connect("collected_star", Callable(self, "_on_star"))
	player.connect("collected_key", Callable(self, "_on_key"))
	player.connect("level_complete", Callable(self, "_on_level_complete"))
...
func load_level(index):
	...
	# maze.tile_data = level.tile_data   # <-- Remove this line
